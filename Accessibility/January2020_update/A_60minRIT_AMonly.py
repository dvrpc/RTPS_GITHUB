...
runDir = r"\\DAISY\BikePedTransit\RTPS\ServiceAccessibility\Jan2020_Update\ModelVersions\Current"
TODs = ["AM"]
...
#just AM
RIT_AM = numpy.zeros(dims)
RIT_AM += RIT["AM"]


#read csv with essiential service counts; read in s strings, so convert to integer to do things with them
with open(r'\\DAISY\BikePedTransit\RTPS\ServiceAccessibility\ES_ByTAZ.csv','rb') as IO:
    r = csv.reader(IO)
...
with open(r'\\DAISY\BikePedTransit\RTPS\ServiceAccessibility\Jan2020_Update\Outputs\Current_60minRIT_AM.csv','wb') as IO:
    w = csv.writer(IO)
    #write the header row
    w.writerow(['Zone_Number','Jobs','ParkTrail','ActivityCenter','Grocery','HealthFac','SchoolU','Count'])    
    #for each origin zone
    for i in xrange(0,len(RIT_AM)):
        ...
        for j in xrange(0, len(RIT_AM[i])):
...
with open(r'\\DAISY\BikePedTransit\RTPS\ServiceAccessibility\Jan2020_Update\Outputs\Current_60minRIT_AM_NoDataZones.csv','wb') as IO:
    w = csv.writer(IO)    
